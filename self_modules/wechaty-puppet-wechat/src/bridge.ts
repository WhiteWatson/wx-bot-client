...
export class Bridge extends EventEmitter {
  ...
  constructor(public options: BridgeOptions) {
    super();
  ...
  public async inject(page: puppeteer.Page): Promise<void> {
    ...
    try {
      ...
      let retObj = (await page.evaluate(sourceCode)) as
        | undefined
        | InjectResult;
  ...
  public async roomModTopic(roomId: string, topic: string): Promise<string> {
    if (!roomId) {
  ...
  public async getContact(
    id: string
  ): Promise<WebContactRawPayload | WebRoomRawPayload> {
    const doGet = async () => {
  ...
  public async forward(
    baseData: WebMessageRawPayload,
    patchData: WebMessageRawPayload
  ): Promise<boolean> {
  ...
  public async cookies(
    cookieList?: puppeteer.Protocol.Network.Cookie[]
  ): Promise<void | puppeteer.Protocol.Network.Cookie[]> {
    if (!this.page) {
      throw new Error("no page");
    }
